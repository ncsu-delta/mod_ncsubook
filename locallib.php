...
function ncsubook_preload_chapters($ncsubook) {
    global $DB;
    $chapters = $DB->get_records('ncsubook_chapters', array('ncsubookid'=>$ncsubook->id), 'pagenum', 'id, pagenum, subchapter, title, hidden');
    if (!$chapters) {
        return array();
    }
    ...
    foreach ($chapters as $id => $ch) {
        ...
        $ch->pagenum = $pagenum;
        if ($first) {
            // ncsubook can not start with a subchapter
            $ch->subchapter = 0;
            $first = false;
        }
        if (!$ch->subchapter) {
...
function ncsubook_get_toc($chapters, $chapter, $ncsubook, $cm, $edit) {
    ...
    if ($edit) { // Teacher's TOC
        ...
        foreach ($chapters as $ch) {
            ...
            $toc .= html_writer::link(new moodle_url('edit.php', array('cmid' => $cm->id, 'chapterid' => $ch->id)),
            ...
            $toc .= html_writer::link(new moodle_url('pre-edit.php', array('cmid' => $cm->id, 'pagenum' => $ch->pagenum)),
                                            $OUTPUT->pix_icon('add', get_string('addafter', 'mod_ncsubook'), 'mod_ncsubook'), array('title' => get_string('addafter', 'mod_ncsubook')));
...
function ncsubook_get_chapter_content($chapterid) {
    ...
    $blockdata = $DB->get_records_sql('select a.*, b.csselementtype from mdl_ncsubook_blocks a inner join mdl_ncsubook_blocktype b on a.type = b.id where chapterid = ? order by a.blockorder', array($chapterid));
    // var_dump($blockdata); die;
    // $blockdata = $DB->get_records('ncsubook_blocks', array('chapterid' => $chapterid), 'blockorder', '*');
    return $blockdata;
}
...
function ncsubook_get_block_class($blocktype) {
    ...
    $blockclass = $DB->get_record('ncsubook_blocktype', array('id' => $blocktype), 'cssclassname');
    return $blockclass->cssclassname;
}

/* function ncsubook_get_block_header($blocktype) {
    global $DB;
    $blockheader = '';

    $blockheader = $DB->get_record('ncsubook_blocktype', array('id' => $blocktype), 'headertext');
    return $blockheader->headertext;
} */
...
function ncsubook_rearrange_content_blocks($blocks) {
    ...
    foreach ($blocks as $block) {
        $current_block_count++;
        if ($block->csselementtype == 'content' && !$content_block_found) {
            $contentblock = $block;
            $content_block_found = true;
        } elseif ($block->csselementtype == 'content' && $content_block_found) {
            $blockarr[] = $contentblock;
            $contentblock = $block;
        } elseif ($block->csselementtype == 'float') {
            $blockarr[] = $block;
        } elseif ($block->csselementtype != 'float' && $content_block_found) {
            $blockarr[] = $contentblock;
            $blockarr[] = $block;
            $contentblock = '';
            $content_block_found = false;
        ...
        if ($current_block_count == $blockcount && $content_block_found) {
            $blockarr[] = $contentblock;
        }
    }
    return $blockarr;
...
function ncsubook_get_latest_pagenum_from_bookid($bookid) {
    ...
    $result = $DB->get_record_sql('select max(pagenum) as maxpagenum from {ncsubook_chapters} where ncsubookid = ?', array($bookid));
...
function ncsubook_add_default_blocks( $chapter ) {
    ...
    if ( empty( $chapter->id ) || ! isset( $chapter->type ) )
        return false;
    ...
    // The types below are block types which are stored in the mdl_ncsubook_blocktype table.
    // End GDH
    switch ($chapter->type) {
        case 5:
...
function ncsubook_get_block_list($chapterid) {
    ...
    $result = $DB->get_records_sql('select id, title from {ncsubook_blocks} where chapterid = ? order by blockorder ASC', array($chapterid));
    return $result;
}
...
function ncsubook_add_block( $block, $chapterid ) {
    ...
    if ( empty( $chapterid ) || ! isset( $block ) )
        return false;

    $max_block_order = ncsubook_get_max_block_order($chapterid);
    foreach ($max_block_order as $key => $value) {
...
function ncsubook_count_content_blocks($chapterid) {
    ...
    $result = $DB->count_records_sql('select count(*) as numcontentblocks from {ncsubook_blocks} where chapterid = ? AND type = ?', array($chapterid, '1'));
    return $result;
}
...
function ncsubook_get_blocktype($blockid) {
    ...
    $result = $DB->get_records_sql('select type from {ncsubook_blocks} where id = ?', array($blockid));
    return $result;
}
...
function ncsubook_get_max_block_order($chapterid) {
    ...
    $result = $DB->get_records_sql('select max(blockorder) as maxblockorder from {ncsubook_blocks} where chapterid = ?', array($chapterid));
    return $result;
}
...
function ncsubook_reorder_blocks ($chapterid) {
    ...
    $blocks = $DB->get_records_sql('select id, blockorder from {ncsubook_blocks} where chapterid = ? order by blockorder ASC', array($chapterid));


    // Cycle through each block record and reorder the blocks 1 thru however many blocks there are
    $new_blockorder_value = 1;
    foreach ($blocks as $block) {
        $block->blockorder = $new_blockorder_value;
        $result = $DB->update_record('ncsubook_blocks', $block);
        $new_blockorder_value++;
    }
    return true;
}
...
function ncsubook_get_chapter_css_tag($chapterid) {
    ...
    $result = $DB->get_records_sql('select b.cssclassname from mdl_ncsubook_chapters a inner join mdl_ncsubook_chaptertype b on a.type = b.id where a.id = ?', array($chapterid));
...
function ncsubook_get_parent_chapter_title($bookid,$chapter_pagenum) {
    global $DB;

    $table = 'ncsubook_chapters';
    $select = 'ncsubookid = ? and pagenum < ? and subchapter = 0';
    $params = array($bookid, $chapter_pagenum);
    $sort = 'pagenum desc';
