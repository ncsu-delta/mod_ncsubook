...
function mod_ncsubook_migrate_moddata_dir_to_legacy($ncsubook, $context, $path) {
    ...
    $base = "$CFG->dataroot/$ncsubook->course/$CFG->moddata/ncsubook/$ncsubook->id";
    $fulldir = $base.$path;
    ...
    foreach ($items as $item) {
        ...
        if ($item->isFile()) {
            ...
            $filepath = clean_param("/$CFG->moddata/ncsubook/$ncsubook->id".$path, PARAM_PATH);
...
function mod_ncsubook_migrate_all_areas() {
    ...
    foreach($rsncsubooks as $ncsubook) {
        ...
        $rschapters = $DB->get_recordset('ncsubook_chapters', array('ncsubookid'=>$ncsubook->id));
